...
pub(crate) struct EguiEditor<T> {
	...
	/// The scaling factor reported by the host, if any. On macOS this will
	/// never be set and we should use the system scaling factor instead.
	...
	T: 'static + Send + Sync,
{
	fn spawn(
		...
		let window = EguiWindow::open_parented(
			...
			move |egui_ctx, _queue, state| {
				...
				// For now, just always redraw. Most plugin GUIs have meters, and those almost
				// always need a redraw. Later we can try to be a bit more sophisticated about
				// this. Without this we would also have a blank GUI when it gets first opened
				// because most DAWs open their GUI while the window is still unmapped.
	...
	fn set_scale_factor(&self, factor: f32) -> bool {
		// If the editor is currently open then the host must not change the current
		// HiDPI scale as we don't have a way to handle that. Ableton Live does this.
	...
	fn param_value_changed(&self, _id: &str, _normalized_value: f32) {
		// As mentioned above, for now we'll always force a redraw to allow
		// meter widgets to work correctly. In the future we can use an
		// `Arc<AtomicBool>` and only force a redraw when that boolean is set.
	}
...
/// The window handle enum stored within 'WindowHandle' contains raw pointers.
...
impl Drop for EguiEditorHandle {
	fn drop(&mut self) {
		...
		// XXX: This should automatically happen when the handle gets dropped, but
		// apparently not
		self.window.close();
	}
}
